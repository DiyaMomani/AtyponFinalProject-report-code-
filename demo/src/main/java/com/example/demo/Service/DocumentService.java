...
public class DocumentService {
    ...
    public String addDocument(String db_name , String collection_name , Map<String,String> prop,
        ...
        if(workerId.charAt(0) < '0' || workerId.charAt(0) > '4')
            workerId = "0";
        URL dist =new URL("http://worker"+workerId+":8080/api/add/record/" + db_name +"/" + collection_name + "/true/true");
        HttpURLConnection conn = (HttpURLConnection) dist.openConnection();
    ...
    @SneakyThrows
    public boolean addStore(String building, String id, String area, String height, HttpSession session){
        UserToken user = (UserToken) session.getAttribute("entered");
        String workerId= jsonService.getWorker(user.getUsername());
        if(workerId.charAt(0) < '0' || workerId.charAt(0) > '4')
            workerId = "0";
    ...
    public boolean isExist(String db_name, String collection, String Id,HttpSession session){
        ...
        if(workerId.charAt(0) < '0' || workerId.charAt(0) > '4')
            workerId = "0";
        URL dist =new URL("http://worker"+workerId+":8080/api/check/" + db_name+ "/" +collection + "/" + Id);
        HttpURLConnection conn = (HttpURLConnection) dist.openConnection();
        conn.setRequestMethod("GET");
        BufferedReader in = new BufferedReader(
                new InputStreamReader(conn.getInputStream()));
