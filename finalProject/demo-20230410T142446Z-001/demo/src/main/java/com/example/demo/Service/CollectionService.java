...
public class CollectionService {
    ...
    public String createCollection(String db_name , String collectionType ,HttpSession session){
        ...
        URL dist =new URL("http://localhost:808"+workerId+"/api/create/collection/" + db_name +"/"+collectionType+"/true/true");
    ...
    public String deleteCollection(String db_name , String collection_name , HttpSession session){
        UserToken user = (UserToken) session.getAttribute("entered");
        String workerId = jsonService.getWorker(user.getUsername());
        URL dist =new URL("http://localhost:8080"+"/api/delete/collection/" + db_name +"/"+collection_name+"/true/true");
        HttpURLConnection conn = (HttpURLConnection) dist.openConnection();
    ...
    public String getList(String db_name,HttpSession session){
        UserToken user = (UserToken) session.getAttribute("entered");
        String workerId = jsonService.getWorker(user.getUsername());
        URL dist =new URL("http://localhost:8080"+"/api/getCollection/" + db_name);
        HttpURLConnection conn = (HttpURLConnection) dist.openConnection();
    ...
    public boolean isExist(String db_name , String collection_name, HttpSession session){
        ...
        String workerId = jsonService.getWorker(user.getUsername());
        URL dist =new URL("http://localhost:808"+workerId+"/worker/db/find/" + db_name + "/" + collection_name);
        HttpURLConnection conn = (HttpURLConnection) dist.openConnection();
        conn.setRequestMethod("GET");
        BufferedReader in = new BufferedReader(
                new InputStreamReader(conn.getInputStream()));
